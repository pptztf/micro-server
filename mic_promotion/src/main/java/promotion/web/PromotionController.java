...
import api.bean.Promotion.CartBean;
import api.core.ApiException;
import api.core.ApiUtil;
...
import promotion.bean.SellerBean;
import promotion.dao.*;
import promotion.domain.*;
...
public class PromotionController {
    ...
    public Map<String, Object> save(@RequestBody Map param) {
      /*  Integer typeId = Integer.valueOf(ApiUtil.toStringAndTrim(param.get("promotionTypeId")));
        Map<String, Object> result = new HashMap<String, Object>();

        if (typeId == 12) {//折扣
            Promotion promotion = new Promotion();
            promotion = (Promotion) ApiUtil.transformMap2Bean(param, promotion);
            ...
            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(pro.getId())),typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
            ...
            fullPromotion = (FullPromotion) ApiUtil.transformMap2Bean(param, fullPromotion);
            FullPromotion full = (FullPromotion) promotionDao.save(fullPromotion);

            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(full.getId())), typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
            result.put("Obj", full);
            result.put("promotionType", typeId);
        } else if (typeId == 28) {//满减
            FullCut fullCut = new FullCut();
            fullCut = (FullCut) ApiUtil.transformMap2Bean(param, fullCut);
            FullCut cut = (FullCut) promotionDao.save(fullCut);

            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(cut.getId())), typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
            result.put("Obj", cut);
            result.put("promotionType", typeId);
        } else if (typeId == 30) { //阶梯满减
            SaveFullLadder ladder = new SaveFullLadder();
            ladder = (SaveFullLadder) ApiUtil.transformMap2Bean(param, ladder);
            SaveFullLadder fullLadder = (SaveFullLadder) promotionDao.save(ladder);
            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(fullLadder.getId())), typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
            result.put("Obj", fullLadder);
            result.put("promotionType", typeId);
        } else if (typeId == 1) {// 赠品促销
            PromotionalGifts promotionalGifts = new PromotionalGifts();
            promotionalGifts = (PromotionalGifts) ApiUtil.transformMap2Bean(param, promotionalGifts);
            PromotionalGifts returnData = (PromotionalGifts) promotionDao.save(promotionalGifts);
            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(returnData.getId())), typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
            result.put("Obj", returnData);
            result.put("promotionType", typeId);
        } else if (typeId == 18) {//套装
            SetPromotion setPromotion = new SetPromotion();
            setPromotion = (SetPromotion) ApiUtil.transformMap2Bean(param, setPromotion);
            SetPromotion set = (SetPromotion) promotionDao.save(setPromotion);
            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(set.getId())),typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
            result.put("Obj", set);
            result.put("promotionType", typeId);
        } else if (typeId == 0) {//满N件减M件
            NmPromotion nmp = new NmPromotion();
            nmp = (NmPromotion) ApiUtil.transformMap2Bean(param, nmp);
            NmPromotion nmpro = (NmPromotion) promotionDao.save(nmp);
            log.info("活动信息：" + nmpro.getId() + "  " + nmpro.getSaleName());
            createPromotion(Integer.valueOf(ApiUtil.toStringAndTrim(nmp.getId())),typeId,Integer.valueOf(ApiUtil.toStringAndTrim(param.get("scopeId"))),ApiUtil.toStringAndTrim(param.get("productJson")));
    ...
    public CartBean calculates(@RequestParam Map map) throws NumberFormatException, ApiException {
        ...
        cb.setProductJson(ApiUtil.toStringAndTrim(map.get("productJson")));
        ...
        if (cb.getPromotionTypeId() == 12) {
            ...
            FullPromotion fullpromotion = (FullPromotion) promotionDao.findNode(Long.parseLong(ApiUtil.toStringAndTrim(cb.getPromotionId())));
            if (cb.isJoin()) {
            ...
            FullCut fullCut = (FullCut) promotionDao.findNode(Long.parseLong(ApiUtil.toStringAndTrim(cb.getPromotionId())));
            if (cb.isJoin()) {
                insert.put("cartBean", cb);
            ...
            SetPromotion setPromotion = (SetPromotion) promotionDao.findNode(Long.parseLong(ApiUtil.toStringAndTrim(cb.getPromotionId())));
            if (cb.isJoin()) {
                insert.put("cartBean", cb);
    ...
    public CartBean calculate(@RequestParam Map map) throws NumberFormatException, ApiException {
        ...
        if (cb.getPromotionTypeId() == 12) {
            ...
            insert.put("cartBean", cb);
            SaveFullLadder saveFullLadder = (SaveFullLadder) promotionDao.findNode(Long.parseLong(ApiUtil.toStringAndTrim(cb.getPromotionId())));
            insert.put("SaveFullLadder", saveFullLadder);
            cb = ruleResult(insert);
        } else if (cb.getPromotionTypeId() == 1) {//满赠促销
            insert.put("cartBean", cb);
            PromotionalGifts promotionalGifts = (PromotionalGifts) promotionDao.findNode(Long.parseLong(ApiUtil.toStringAndTrim(cb.getPromotionId())));
