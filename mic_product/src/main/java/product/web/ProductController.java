   ...
   private ProductDao productDao;
    ...
    public Iterable queryProduct(@RequestParam Map<String,Object> parms){
        ...
        if(!Util.isNullOrEmpty(parms.get("categoryId"))){
            ...
            iterable=neo4jOperations.queryForObjects(Product.class,cypher, new HashMap());
            //rs.add(iterable);
            //	map.put("result", rs);
        }
        //根据产品id查询
            ...
            sb.append(") return n");
            iterable =neo4jOperations.queryForObjects(Product.class,sb.toString(), new HashMap());
            //map.put("result", rs);
        }
        //只根据店家查询（不包括产品类型）
    ...
    public Iterable   queryTemplate(@RequestParam Map<String,Object> parms){
        ...
        if(!Util.isNullOrEmpty(parms.get("categoryId"))){
            ...
            iterable = neo4jOperations.queryForObjects(Template.class, cypher, new HashMap());
    ...
    public Template createTemplate(@RequestBody TemplateBean templateBean){
        Map map = ApiUtil.transBean2MapNotNull(templateBean);
    ...
    public Template updTemplate(@PathVariable Long id, @RequestBody TemplateBean templateBean){
        ...
        neo4jOperations.save(templateEntity);
        return templateEntity;
    }
